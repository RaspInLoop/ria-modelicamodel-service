...
import org.raspinloop.server.modelica.mdt.core.Element;
...
import org.raspinloop.server.modelica.mdt.core.IllegalRestrictionException;
import org.raspinloop.server.modelica.mdt.core.List;
import org.raspinloop.server.modelica.mdt.core.ListElement;
import org.raspinloop.server.modelica.mdt.core.ModelicaParserException;
import org.raspinloop.server.modelica.mdt.core.compiler.ComponentParser;
import org.raspinloop.server.modelica.mdt.core.compiler.ElementInfo;
...
import org.raspinloop.server.modelica.mdt.core.compiler.ModelicaParser;
...
public class OMCProxyWrapper implements IModelicaCompiler {
	...
	SmartProxy smartProxy = new SmartProxy("Raspinloop", "Modelica", log.isTraceEnabled(), log.isTraceEnabled());
	 ...
	 * @return a String[] {result, answerToGetErrorString}; the server
	 */
	// TODO add synchronization so that two threads don't fudge up each others
	// communication with OMC
	// old synchronization aka 'private synchronized String
	// sendExpression(String command)'
	// doesn't work when there is possibility of multiple instances of OMCProxy
	// objects
	@Override  	
	public ICompilerResult sendExpression(String command, boolean showInConsole) throws ConnectException {
		if (cacheManager != null){
			Cache cache = cacheManager.getCache( "CompilerProxy.command");
			ValueWrapper found;
			if (cache != null){
				found = cache.get(command);
			if (found != null){ 
				log.trace("return cached value for {} ", command);
				return CompilerResult.makeResult((String[])found.get(), "");
			}
			}
		}
		Result result = smartProxy.sendExpression(command);
		ICompilerResult cres = CompilerResult.makeResult(result);
		if (StringUtils.isBlank(cres.getError()) && cacheManager != null){
			Cache cache = cacheManager.getCache( "CompilerProxy.command");			
			if (cache != null){
				log.trace("Put result of command {} in cache ", command);
				cache.put(command, cres.getResult());
				}
			}
	...
	public ICompilerResult sendExpressionNoCache(String command, boolean showInConsole) throws ConnectException {		
		Result result = smartProxy.sendExpression(command);
	 ...
	 * @return the type of restriction of the class or Type.CLASS if type can't
	 *         be determined
	 * @throws ConnectException
	 * @throws UnexpectedReplyException
	 */
	@Override
	public IModelicaClass.Restriction getRestriction(String className) throws ConnectException, UnexpectedReplyException {
	 ...
	 * @param retval,
	 *            the string returned by the OMC compiler
	...
	public ParseResults loadSourceFile(File file) throws ConnectException, UnexpectedReplyException {
		...
		ICompilerResult ret = sendExpressionNoCache("loadFileInteractiveQualified(\"" + fullName + "\")", false);
		...
		if (isError(errorString)) {
			...
			/*
			 * If there were errors, but the compilation went through, collect
			 * the error messages. (Test if errorString != "")
			 */
	 ...
	 */
	@Override
	public DefinitionLocation getClassLocation(String className) throws ConnectException, UnexpectedReplyException, InvocationError {
		 ...
		 *
		 * {<file path>,<something>,<start line>,<start column>,<end line>,<end
		 * column>}
		 *
		 * for example: {/foo/Modelica/package.mo,writable,1,1,1029,13}
		 */
		...
		try {
			...
			throw new UnexpectedReplyException("Can't parse getCrefInfo() reply, " + "unexpected format");
		}
	...
	public Collection<ElementInfo> getElements(String className) throws ConnectException, InvocationError, UnexpectedReplyException {
		...
		String retval = res.getFirstResult();
		/*
		 * we need a efficient way to check if the result is humongosly huge
		 * list or 'Error' or maybe 'error'
		 */
		for (int i = 0; i < retval.length(); i++) {
			if (retval.charAt(i) == '{') {
				...
				/*
				 * this is the unreadable way to check if the retval equals
				 * 'Error' or 'error'
				 */
	...
	public String[] getStandardLibrary() throws ConnectException {
		...
		for (int i = 0; i < omcLibraries.length; i++) {
			sendExpressionNoCache("loadModel(" + omcLibraries[i] + ")", false);
		}
	...
	String evaluateExpressionLocally(String command) {
		...
		if (command.equals("traceError")) {
			...
			retval = "Tracing of errors while talking with OpenModelica is set to: " + traceErrorPreferences;
		}
	...
	public boolean existClass(String className) throws ConnectException, UnexpectedReplyException {
		ICompilerResult res = sendExpressionNoCache("existClass(" + className + ")", false);
	...
	public ICompilerResult loadFile(String classPath) throws ConnectException, UnexpectedReplyException {
		...
		ICompilerResult res = sendExpressionNoCache("loadFile(\"" + classPath + "\")", false);
		return res;
	}
	...
	public List parseFile(String fileName) throws ConnectException, UnexpectedReplyException {
		ICompilerResult retval = sendExpressionNoCache("parseFile(\"" + fileName + "\")", false);
	...
	public ICompilerResult buildModel(String className) throws ConnectException, UnexpectedReplyException {
		ICompilerResult res = sendExpressionNoCache("buildModel(" + className + ")", false);
		return res;
	}
	...
	/**
	 * Instantiates a model/class and returns a string containing the flat class
	 * definition. Ex: instantiateModel(dcmotor)
	 *
	 * @param modelname
	 *            the modelname
	...
	public String instantiateModel(String modelname) throws ConnectException {
		ICompilerResult res = sendExpressionNoCache("instantiateModel(" + modelname + ")", false);
		String retval = res.getFirstResult();
		return retval;
	}
	...
	public ICompilerResult searchClassNames(String className) throws ConnectException {
		ICompilerResult res = sendExpressionNoCache("searchClassNames(" + className + ")", false);
		return res;
	}
	...
	public String getIconAnnotation(String className) throws ConnectException {
		ICompilerResult res = sendExpression("getIconAnnotation(" + className + ")", false);
		String retval = res.getFirstResult();
		return retval;
	}
	...
	public String getDiagramAnnotation(String className) throws ConnectException {
		ICompilerResult res = sendExpression("getDiagramAnnotation(" + className + ")", false);
		String retval = res.getFirstResult();
		return retval;
	}
	...
	public ArrayList<String> getComponentAnnotations(String className) throws ConnectException, ModelicaParserException {
		ICompilerResult res = sendExpression("getComponentAnnotations(" + className + ")", false);
		List list = ModelicaParser.parseList(res.getFirstResult());
		ArrayList<String> retval = new ArrayList<>();
		Iterator<ListElement> it = list.iterator();
		while (it.hasNext()) {
			ListElement type = it.next();
			Iterator<ListElement> elementIt = ((List) type).iterator();
			if (elementIt.hasNext()) {
				ListElement element = elementIt.next();
				retval.add(((Element) element).toString());
			} else {
				retval.add("");
			}
		}
